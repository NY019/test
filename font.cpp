...
BOOL FontAdd(VOID)
{
	...
	if (AddFontResourceEx(FONT_HOZUKI_PATH, FR_PRIVATE, NULL) == 0)
	{
		MessageBox(GetMainWindowHandle(), FONT_HOZUKI_NAME, FONT_INSTALL_ERR_TITLE, MB_OK);
		return FALSE;
	}
...
BOOL FontCreate(VOID)
{
	...
	//フォントハンドルを作成
	if (FontCreateHandle(&Hozuki) == FALSE) { return FALSE; }
...
VOID FontDelete(VOID)
{
	...
	DeleteFontToHandle(Hozuki.handle);
